...
import type { DashboardRecentOrder, DashboardStats } from '../../services/api/dashboardApi/dashboardApi'
...
export default memo(function DashboardPage() {
  ...
  const refreshDashboard = useCallback(async () => {
    try {
      const data = await appService.dashboard.summary()
      setStats(data.stats)
  ...
  useEffect(() => {
    const bump = () => void refreshDashboard()
    const offOrders = onOrdersChanged(bump)
    const offCustomers = onCustomersChanged(bump)
    return () => {
      offOrders()
      offCustomers()
    }
