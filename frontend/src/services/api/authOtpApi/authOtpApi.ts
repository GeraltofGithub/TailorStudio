...
export async function otpLoginResend(email: string, password: string): Promise<OtpSendOk> {
  return api._post<OtpSendOk>('/api/auth/otp/login/resend', { email, password })
}

export async function otpLoginComplete(email: string, password: string, code: string): Promise<MeResponse> {
  const r = await api._post<{ ok: true; me: MeResponse; accessToken?: string }>('/api/auth/otp/login/complete', {
    email,
    password,
    code,
  })
